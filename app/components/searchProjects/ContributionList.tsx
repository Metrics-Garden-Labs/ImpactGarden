...
import React, {
  ...
  import { ContributionWithProjectsAndAttestationCount } from "@/src/types";
  ...
  const ContributionList: React.FC<Props> = ({
    ...
    const filteredContributions = useMemo(() => {
      return contributions.filter((contribution) => {
        if (
          query &&
          !contribution.contribution?.toLowerCase().includes(query.toLowerCase()) &&
    ...
    const sortedContributions = useMemo(() => {
      ...
      switch (sortOrder) {
        ...
        case "Z-A":
          sorted.sort((a, b) =>
            (b.contribution || "").localeCompare(a.contribution || "", undefined, {
              sensitivity: "base",
            })
          );
          break;
        ...
        default:
          sorted.sort((a, b) =>
            (a.contribution || "").localeCompare(b.contribution || "", undefined, {
              sensitivity: "base",
            })
          );
      }
    ...
    const isReviewedContribution = (projectName: string) => {
      return userAttestations.some(
        (attestation) => attestation.projectName?.toLocaleLowerCase() === projectName.toLocaleLowerCase()
      );
    };
        ...
        <div className="grid grid-cols-1 gap-4 mx-3 md:grid-cols-3 md:mx-8 lg:grid-cols-4 lg:gap-12 max-w-6xl overflow-y-auto">
          {isFiltering
            ...
            : sortedContributions
                ...
                .map((contribution) => {
                  const isReviewed = isReviewedContribution(contribution.projectName || "");
